 ...
 *
 * Photoshop 2026 answers every geometry question with zero, for every element,
 * through every API:
 *
 *   #app: rect=0x0@0,0 client=0x0 offset=0x0 | #preview-wrap: rect=0x0@0,0
 *   client=0x0 offset=0x0 | #scroll: ... | window=undefinedxundefined
 *
 * so this is not a theoretical branch, it is what the plugin ran on. A guess is
 ...
 *
 * The width errs high, at the top of the range a panel is likely to be. The two
 * ways of being wrong are not symmetrical: rendering wider than the panel gets
 * scaled down by CSS and costs only render time, while rendering narrower gets
 * scaled *up* and is visibly soft - which looks like a broken preview rather
 * than like a guess.
 */
...
class Panel {
  ...
  bindPreviewGrip() {
    ...
    const end = (e) => {
      ...
      // Landing on the floor is how a preview gets stuck, and the way back is
      // a double-click on a bar with no label. Said once, when it is relevant.
      if (this.ui.previewHeight === PREVIEW_HEIGHT_MIN && !this._floorHinted) {
        this._floorHinted = true;
        this.notice("The preview is at its smallest. Double-click this bar to go back to automatic.");
      }
      this.persistSession();
    };
...
function sanitizeUI(raw) {
  ...
  if (raw && raw.previewHeight) {
    /*
     * A stored height at the floor is treated as an accident, not a choice.
     *
     * The floor is where a drag lands when it clamps, and on a host that
     * reports no panel size the drag's limits are computed from a height it
     * never gave - so landing there is easy and getting back is not obvious.
     * The previous attempt at this dropped anything at or under 110 and was
     * still too narrow: the panel came back reporting a box of 360x150, which
     * is a stored height somewhere in 111..150 being clamped straight onto the
     * floor again. Nobody chooses the smallest size the control allows, so the
     * floor itself is the right threshold. Anything above it is a real choice
     * and is kept.
     */
    if (raw.previewHeight > PREVIEW_HEIGHT_MIN) {
